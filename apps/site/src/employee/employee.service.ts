...
export class EmployeeService {
  ...
  async update(id: number, updateEmployeeDto: UpdateEmployeeDto): Promise<EmployeeResponseDto> {
    ...
    const updatedEmployee = await this.employeeRepository.findOne({ where: { id: updateEmployeeDto.id } });
  ...
  private toResponseDto(employee: Employee): EmployeeResponseDto {
    ...
    const { id, username,password, name, lastName, department, role, email, phone, workPhone, isActive, gender, address, familyName, birthDate, major, province, district } = employee;
    return { id, username,password, name, lastName, department, role, email, phone, workPhone, isActive, gender, address, familyName, birthDate, major, province, district };
  }
}